...
def reqweather(api_urn = "https://api.weather.gov", weather_station = "BGM", weather_station_xlocation = "63", weather_station_ylocation = "60",  ):
    api_url_forecast = "/gridpoints/"+weather_station+"/"+weather_station_xlocation+","+weather_station_ylocation+"/forecast"
    api_forecast_results = requests.get(api_urn+api_url_forecast).json()  # Make API call

    tomorrow_forecast_list = [api_forecast_results["properties"]["periods"] for api_forecast_results["properties"]["periods"] in api_forecast_results["properties"]["periods"] if api_forecast_results["properties"]["periods"]["number"] == 2]
